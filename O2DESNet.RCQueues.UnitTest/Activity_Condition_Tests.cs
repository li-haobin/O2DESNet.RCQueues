...
namespace O2DESNet.RCQueues.UnitTest
{
    ...
    public class DynamicLoadAttributeUpdateSimulator : RCQsContext
    {
        public new class Statics : RCQsContext.Statics
        {
            public readonly List<ConditionalActivityLoad> Loads;
            ...
            public Statics()
            {
                ...
                Loads = new List<ConditionalActivityLoad>
                {
                    new ConditionalActivityLoad {Id = "First"},
                    new ConditionalActivityLoad {Id = "Second"},
                    new ConditionalActivityLoad {Id = "Third"},
                };
            }
        }
        ...
        public Dictionary<string, string> LoadProcessorMap = new Dictionary<string, string>();

        public DynamicLoadAttributeUpdateSimulator(Statics config, int seed = 0) : base(config, seed)
        {
            Config = config;
            ProcessedLoad = new Dictionary<string, List<ConditionalActivityLoad>>();

            LoadProcessorMap["First"] = "Processor 1";
            LoadProcessorMap["Second"] = "Processor 2";
            LoadProcessorMap["Third"] = "Processor 1";
            ...
            Queue = new ActivityHandler<ConditionalActivityLoad>(queueStatic, DefaultRS.Next());
            // Update load attribute after the queue activity completed.
            Queue.FlowTo(load => load.Processor = LoadProcessorMap[load.Id]);
            AddChild(Queue);
    ...
    public class Activity_Condition_Tests
    {
        ...
        [Test]
        public void RunDynamicLoadAttributeUpdate()
        {
            DynamicLoadAttributeUpdateSimulator simulator = new DynamicLoadAttributeUpdateSimulator(new DynamicLoadAttributeUpdateSimulator.Statics());
            simulator.Run(DateTime.Today.AddDays(1));
